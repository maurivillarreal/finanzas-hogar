...
export default function EditarIngreso() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [{ data: ingreso }, { data: perfilesData }, { data: cuentasData }] = await Promise.all([
        supabase.from('ingresos').select('*').eq('id', id).single(),
        supabase.from('perfiles').select('*'),
        supabase.from('cuentas').select('*').eq('activa', true).order('nombre'),
      ])

      if (!ingreso) { router.push('/ingresos'); return }
      ...
      setForm({
        ...
        notas: ingreso.notas || '',
        para_perfil_id: ingreso.para_perfil_id || ingreso.perfil_id,
        recurrente: ingreso.recurrente || false,
      })
      setMontoDisplay('$' + Math.round(ingreso.monto).toLocaleString('es-AR'))
      setLoading(false)
    }
    init()
  }, [id, router])
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <header className="bg-gray-900 border-b border-gray-800 px-4 py-4">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          ...
          <h1 className="text-lg font-bold text-yellow-400">Editar ingreso</h1>
          <div className="w-16" />
        </div>
      </header>
